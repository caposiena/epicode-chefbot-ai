...
Per la classificazione ho utilizzato MobileNetV2 con i pesi già appresi su ImageNet. Nel primo notebook la parte convoluzionale viene inizialmente congelata e vengono addestrati i nuovi livelli finali. In una seconda fase vengono sbloccati gli ultimi venti livelli della rete per effettuare un breve fine-tuning con un learning rate più basso.

Il modello finale ha ottenuto un'accuracy del 81,47% su 3.000 immagini di test, con 250 immagini per ciascuna classe. L'F1-score medio è del 81,5%. Le categorie riconosciute meglio sono spaghetti alla carbonara, pizza, paella e insalata greca. Le confusioni più frequenti riguardano piatti con un aspetto simile, per esempio bolognese e carbonara, caprese e bruschetta, panna cotta e cheesecake, ravioli e lasagna.

La seconda parte del progetto riguarda la ricerca semantica. L'utente può scrivere una frase come “vorrei qualcosa di fresco e leggero” oppure “cerco un dolce cremoso”. La frase e le descrizioni dei piatti vengono trasformate in embeddings attraverso un modello Sentence Transformers multilingua. Il confronto viene eseguito usando la similarità coseno e restituisce i piatti semanticamente più vicini alla richiesta, senza limitarsi alla ricerca di parole identiche.
...
Food101 viene scaricato automaticamente da TensorFlow Datasets durante la prima esecuzione del notebook. Il modello addestrato è presente nella cartella models insieme al file che conserva l'ordine delle classi. L'applicazione può essere avviata dalla cartella principale con il comando streamlit run app.py. È stata provata anche su un computer senza GPU; in questo caso il primo caricamento del modello richiede semplicemente qualche secondo in più.
